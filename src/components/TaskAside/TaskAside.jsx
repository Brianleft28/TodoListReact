...
const TaskAside = () => { 
  ...
  return (
    ...
    <motion.aside
        ...
        <div className='flex justify-center  md:items-center mb-6'>
            ...
            setDescription={ setDescription }/>
                <button className="fixed bottom-2 right-12 w-52 justify-center  btn btn-outline btn-secondary px-4 py-2" onClick={ handleToggle }>
                    { isOpen ? 'Cerrar' : '...' } 
                </button>
        </div>
    </motion.aside>
    { //
        !isOpen && (
    <button onClick={handleToggle} className="btn btn-primary fixed bottom-6 right-10">Agregar tarea</button>
    )
    }
    </div>
